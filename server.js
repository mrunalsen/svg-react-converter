...
const fetchSVGsForProject = async (projectId, page, perPage, sort) => {
    ...
    const response = await axios.post(`http://172.16.1.169:8080/api/project/${projectId}/icons`, {
    ...
    for (const icon of icons) {
        for (const iconImage of icon.iconImages) {
            const svgResponse = await axios.get(`http://172.16.1.169:8080/${iconImage.iconImagePath}`, {
                responseType: 'text'
            });
